...
export async function model(gl, n = 20, graph, source, goal, path) {
	...
	const movement = new Movement(speed, path, n, coordinate.WIDTH / speed, graph, goal);
	const programsInfo = shaders.map((shader) =>
		createProgramInfo(gl, [shader.vertex, shader.fragment])
	);
	// eslint-disable-next-line @typescript-eslint/ban-ts-comment
	// @ts-ignore
	return [
